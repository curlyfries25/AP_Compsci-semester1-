...
class starter {
	...
	*/
	public static void printsNumber(String[] arr, int num){
		...
		if(num > arr.length){
			System.out.println("This number is too large, try again.");
			return;
		}
	...
	*/
	...
	/* ---------------------------------------------------------------------------------------
		The mostCommon method finds the most common word in the given array.      - if ___ word shows up, i++, return said word
		It returns the String of the most common word.                            when ___ word shows up, add it up, if there's another word, replace it
	...
	*/
	public static String mostCommon(String[] arr){
		...
		for(int i = 0; i < arr.length; i++){
			int current = checksArray(arr, arr[i]);
			if(current > max){
				max = current;
				common = arr[i];
			}
		...
		//use checks array and max method
		return common;
	}
